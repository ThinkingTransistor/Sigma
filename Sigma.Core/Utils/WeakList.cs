...
namespace Sigma.Core.Utils
{
	...
	public class WeakList<T> : IList<T>
	{
		...
		{
			return _innerList.Any(wr => ReferenceEquals(wr.Target, item));
		}
		...
		{
			get { return _innerList[index].Target; }
			set { _innerList[index] = new WeakReference<T>(value); }
		}

		/// <summary>
		///		Get the first element that is still referenced - warning, you will recieve a strong reference!
		/// </summary>
		/// <returns>The first element availble - the default value if not existent.</returns>
		public T FirstAvailable()
		{
			while (_innerList.Count > 0)
			{
				WeakReference<T> val = _innerList[0];
				T target = val.Target;
				if (val.IsAlive)
				{
					return target;
		...
		[Serializable]
		public class WeakReference<TWeak> : WeakReference
		{
			public WeakReference(TWeak target) : base(target) { }

			public WeakReference(TWeak target, bool trackResurrection) : base(target, trackResurrection) { }

			public WeakReference(SerializationInfo info, StreamingContext context) : base(info, context) { }

			public new TWeak Target => (TWeak)base.Target;
		}
	}
}
