...
namespace Sigma.Core.Training.Optimisers.Gradient.Memory
{
	...
	{
		protected readonly string MemoryIdentifier;
		...
		{
			return !IsInMemory(paramIdentifier) ? defaultValue : Registry.Get<Dictionary<string, TMemory>>(MemoryIdentifier)[paramIdentifier];
		}
		...
		{
			return !IsInMemory(paramIdentifier) ? defaultValueFunction.Invoke() : Registry.Get<Dictionary<string, TMemory>>(MemoryIdentifier)[paramIdentifier];
		}
		...
		protected TMemory SetMemory(string paramIdentifier, TMemory value)
		{
			...
			{
				Registry.Get<Dictionary<string, TMemory>>(MemoryIdentifier).Add(paramIdentifier, value);
			}
			else
			{
				Registry.Get<Dictionary<string, TMemory>>(MemoryIdentifier)[paramIdentifier] = value;
			}
		...
		{
			return Registry.Get<Dictionary<string, TMemory>>(MemoryIdentifier).ContainsKey(paramIdentifier);
		}
		...
		{
			Registry.Get<Dictionary<string, TMemory>>(MemoryIdentifier).Clear();
		}
	}
}
