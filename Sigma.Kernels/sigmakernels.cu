...
__global__ void Permute_M(const float* a, const float* permutedDimensions, const float* originalStrides, 
	float* out, const float* permutedStrides, const int rank, const int n)
{
	extern __shared__ float sdata[];
